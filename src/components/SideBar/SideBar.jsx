    ...
    <aside className="w-[20rem] h-screen bg-stone-200 shadow-2xl">
      <div className="flex flex-col gap-2 justify-center items-center">
        ...
        <div className=" flex flex-col mt-4 gap-2 text-center">
          <button className="py-2 px-16 rounded-md bg-stone-300 text-xl hover:bg-stone-400 hover:text-stone-700">&#43;</button>
          <p className="text-stone-500">Add new project</p>
        </div>
      </div>
      ...
      {projectList <= 0 ? (
        ...
        <ul className="flex flex-col gap-2 mt-16">
          {projectList.map((project) => {
            return (
              <button key={project.id} className="text-lg py-2 text-stone-500 hover:text-stone-700 focus:bg-stone-300 focus:text-stone-700">
                {project.name}
              </button>
            );
          })}
